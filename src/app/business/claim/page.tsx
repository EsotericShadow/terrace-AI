      ...
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-2xl mx-auto px-4">
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-4">Claim Request Submitted</h1>
                   <p className="text-gray-600 mb-6">
                     Thank you for your interest in claiming your business listing{selectedBusinesses.length > 1 ? 's' : ''}. We'll contact you at the phone number you provided within 1-2 business days to verify your ownership before approving your claim.
                   </p>
                     ...
                     <div className="flex items-start space-x-3">
                       ...
                       <div>
                         ...
                         <p className="text-sm text-yellow-800">
                           We'll call you to verify that you're the legitimate owner of the business{selectedBusinesses.length > 1 ? 'es' : ''} before granting you access to manage the listing{selectedBusinesses.length > 1 ? 's' : ''}.
                         </p>
                       </div>
                     </div>
                   </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="flex items-center space-x-3 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-600">Take control of your business listing on Terrace AI</p>
            </div>
          </div>
                   ...
                   <div className="flex items-start space-x-3">
                     ...
                     <div>
                       ...
                       <ul className="text-sm text-blue-800 space-y-1">
                         <li>• <strong>Free listing:</strong> Your business is already visible to Terrace residents</li>
                         <li>• <strong>Pro plan:</strong> $30/month per business for enhanced features</li>
                         <li>• <strong>Multiple businesses:</strong> Each business needs its own Pro subscription</li>
                         <li>• <strong>Verification required:</strong> We'll contact you to verify ownership before approval</li>
                         <li>• <strong>Removal:</strong> Free to remove your listing anytime (verification required)</li>
                       </ul>
                     </div>
                   </div>
                 </div>
                   ...
                   <div>
                     <div className="flex items-center justify-between mb-2">
                       <label htmlFor="business-search" className="block text-sm font-medium text-gray-700">
                         Search for Your Business{multipleBusinesses ? 'es' : ''}
                       </label>
                       <button
                         type="button"
                     ...
                     {multipleBusinesses && (
                       <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                         <p className="text-sm text-yellow-800">
                           <strong>Multiple Business Pricing:</strong> Each business requires its own $30/month Pro subscription. 
                           You can select multiple businesses now and we'll discuss pricing options when we contact you.
                         </p>
                       </div>
                     )}
                         ...
                         {searchResults.map((business) => (
                           <div
                             ...
                             className={`p-3 cursor-pointer hover:bg-gray-50 border-b border-gray-100 last:border-b-0 ${
                               selectedBusinesses.includes(business.id) ? 'bg-terrace-50 border-terrace-200' : ''
                             }`}
                               ...
                               {multipleBusinesses && (
                                 <div className="mt-1">
                                   <input
                                     ...
                                     className="h-4 w-4 text-terrace-600 focus:ring-terrace-500 border-gray-300 rounded"
                                   />
                                 </div>
                               )}
                     ...
                     {selectedBusinesses.length > 0 && (
                       <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                         <div className="text-sm text-green-800 mb-2">
                           ✓ Selected {selectedBusinesses.length} business{selectedBusinesses.length > 1 ? 'es' : ''}:
                         </div>
                         <div className="space-y-1">
                           {selectedBusinesses.map(businessId => {
                             const business = searchResults.find(b => b.id === businessId);
                             return business ? (
                               <div key={businessId} className="text-sm text-green-700">
                                 • {business.name} ({business.category})
                               </div>
                             ) : null;
                           })}
                         </div>
                         {multipleBusinesses && selectedBusinesses.length > 1 && (
                           <div className="mt-2 text-xs text-green-600">
                             Total: ${selectedBusinesses.length * 30}/month for Pro features
                           </div>
                         )}
                       </div>
                     )}
                ...
                <p className="text-red-800 text-sm">{error}</p>
                   ...
                   <div className="flex space-x-4">
                     <button
                       ...
                       className="flex-1 bg-terrace-600 text-white py-2 px-4 rounded-lg hover:bg-terrace-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                     >
                       {isSubmitting ? 'Submitting...' : `Submit Claim Request${selectedBusinesses.length > 1 ? ` (${selectedBusinesses.length} businesses)` : ''}`}
                     </button>
              <Link
                href="/"
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <h3 className="font-medium text-gray-900 mb-3">Need to remove your listing instead?</h3>
            <Link
              href="/business/remove"
              className="text-terrace-600 hover:text-terrace-700 text-sm font-medium"
            >
              Request removal (free) →
            </Link>
          </div>
        </div>
      </div>
